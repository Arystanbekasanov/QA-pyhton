...
def create_ec2_instance(sg, key, user_data):
    instance = ec2_resource.create_instances(ImageId='ami-f2d3638a', MinCount=1, MaxCount=1, SecurityGroups=[sg], KeyName=key, UserData=user_data, InstanceType='t2.micro')
    return instance
...
elb_client.register_instances_with_load_balancer(LoadBalancerName='lb3', Instances=[{'InstanceId': instanceid1[0].id}, {'InstanceId': instanceid2[0].id}])
